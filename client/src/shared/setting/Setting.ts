...
export class Setting {
	...
	constructor(config: Config | ConfigObject, options: SettingOptions = {}, key: string = '') {
	...
	sub(key: string, options: SettingOptions = {}, config?: Config | ConfigObject): Setting {
		const resolvedKey = String(this.config?.resolveValue(key || '') || '').toLowerCase()

		if (!this._[resolvedKey]) {
			const subConfig = config ?? this.config
			const subSetting = new Setting(subConfig as Config | ConfigObject, options, [this._key, key].join('.'))
	...
	useDisclosure(name: string, initial: boolean, options?: UseDisclosureOptions): [boolean, UseDisclosureHandlers] {
	...
	useSetState<T extends Record<string, unknown>>(
		name: string,
		initial?: T
	): [
		T,
		(
			state: { [P in keyof T]?: T[P] | undefined } | ((currentState: T) => { [P in keyof T]?: T[P] | undefined })
		) => void
	] {
